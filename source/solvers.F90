...
!-----------------------------------------------------------------------
   ...
   if (my_task < distrb_tropic%nprocs) then
      ...
      case (solv_cg1)
         call pcg_chrongear(P_TROPIC,RHS_TROPIC) ! Chron-Gear pcg solver
      case (solv_pcg)
...
!-----------------------------------------------------------------------
   ...
   if (my_task == master_task) then
      ...
      write(stdout,blank_fmt)
      write(stdout,*) 'solver_nml namelist settings:'
      write(stdout,blank_fmt)
      write(stdout,solver_nml)
...
!-----------------------------------------------------------------------
      ...
      do iblock=1,nblocks_tropic
         ...
         call btrop_operator(AZ,Z,this_block,iblock)
...
!-----------------------------------------------------------------------
      ...
      do iblock=1,nblocks_tropic
         ...
         if (mod(m,solv_ncheck) == 0) then
            ...
            call btrop_operator(Z,X,this_block,iblock)
            R(:,:,iblock) = B(:,:,iblock) - Z(:,:,iblock)
...
!-----------------------------------------------------------------------
      ...
      if (mod(m,solv_ncheck) == 0) then
         ...
         rr = global_sum(WORK0, distrb_tropic, field_loc_center, &
                         RCALCT_B)  !(r,r)
   ...
   if (solv_sum_iters == solv_max_iters) then
      if (solv_convrg /= c0) then
         write(noconvrg,'(a28,i5)') & 
             ' no convergence at time step ', nsteps_total
...
!-----------------------------------------------------------------------
      ...
      do iblock=1,nblocks_tropic
         ...
         if (lprecond) then
            ...
            do i=1,nx_block
               if (A0(i,j,iblock) /= c0) then
                  WORK1(i,j,iblock) = R(i,j,iblock)/A0(i,j,iblock)
               else
                  WORK1(i,j,iblock) = c0
               endif
            end do
