...
module abio_dic_dic14_mod
  ...
  public :: abio_dic_dic14_tracer_cnt
  public :: abio_dic_dic14_init
  public :: abio_dic_dic14_tracer_ref_val
  public :: abio_dic_dic14_set_sflux
  public :: abio_dic_dic14_tavg_forcing
  public :: abio_dic_dic14_set_interior
  public :: abio_dic_dic14_write_restart

  !-----------------------------------------------------------------------
  !  module variables required by passive_tracers
  !-----------------------------------------------------------------------

  integer (int_kind), parameter :: abio_dic_dic14_tracer_cnt = 2

  !-----------------------------------------------------------------------
  !  relative tracer indices
  !-----------------------------------------------------------------------

  integer (int_kind), parameter :: abio_dic_ind   =  1 ! Abiotic DIC
  integer (int_kind), parameter :: abio_dic14_ind =  2 ! Abiotic DIC14
...
contains
  ...
  subroutine abio_dic_dic14_init(init_ts_file_fmt, read_restart_filename, &
    ...
    call get_timer(abio_dic_dic14_sflux_timer, 'ABIO_DIC_SFLUX', 1, distrb_clinic%nprocs)
  ...
  subroutine abio_dic_dic14_init_tavg()
    ...
    !  local variables
    !-----------------------------------------------------------------------
    integer (int_kind) :: var_cnt             ! how many surface tavg variables are defined?
    !-----------------------------------------------------------------------
  ...
  subroutine abio_dic_dic14_set_sflux( &
       U10_SQR,                        &
       IFRAC,                          &
       PRESS,                          &
       SST,                            &
    ...
    real (r8), dimension(nx_block,ny_block,abio_dic_dic14_tracer_cnt,max_blocks_clinic) , intent(in)    :: SURF_VALS_OLD
    real (r8), dimension(nx_block,ny_block,abio_dic_dic14_tracer_cnt,max_blocks_clinic) , intent(in)    :: SURF_VALS_CUR
    real (r8), dimension(nx_block,ny_block,abio_dic_dic14_tracer_cnt,max_blocks_clinic) , intent(inout) :: STF_MODULE
...
99  call broadcast_scalar(stat, master_task)
  ...
  subroutine comp_varying_CO2(iblock, data_ind_co2, pCO2)
    ...
    integer (int_kind) , intent(out) :: data_ind_co2              ! data_ind_co2 is the index for the data for current timestep,
    real (r8)          , intent(out) ::  pCO2(nx_block, ny_block) ! atmospheric CO2 mole fraction (pmol/mol)

    !-----------------------------------------------------------------------
    !  local variables
    !-----------------------------------------------------------------------
    integer (int_kind) :: i, j        ! loop indices
    real (r8)          :: model_date  ! date of current model timestep
    real (r8)          :: mapped_date ! model_date mapped to data timeline
    real (r8)          :: weight      ! weighting for temporal interpolation
    !-----------------------------------------------------------------------
  ...
  subroutine comp_varying_D14C(iblock, data_ind_d14c, D14C)
    ...
    integer (int_kind) , intent(in)  :: iblock                   ! block index
    integer (int_kind) , intent(out) :: data_ind_d14c            ! data_ind_d14c is the index into data for current timestep,
