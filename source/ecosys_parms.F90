MODULE ecosys_parms
  ...
  real(kind=r8), parameter ::    &
      ...
      loss_thres_diat   = 0.01_r8, & !diat conc. where losses go to zero
...
20     CONTINUE
    ...
    CALL broadcast_scalar(parm_prod_dissolve, master_task)
    CALL broadcast_scalar(parm_o2_min, master_task)
    CALL broadcast_scalar(parm_no3_min, master_task)
    CALL broadcast_scalar(parm_Rain_CaCO3, master_task)
    ...
    IF (my_task == master_task) THEN
       ...
       WRITE (stdout,*) 'parm_prod_dissolve  = ', parm_prod_dissolve
       WRITE (stdout,*) 'parm_o2_min         = ', parm_o2_min
       WRITE (stdout,*) 'parm_no3_min        = ', parm_no3_min
       WRITE (stdout,*) 'parm_Rain_CaCO3     = ', parm_Rain_CaCO3
