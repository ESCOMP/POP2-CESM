...
!-----------------------------------------------------------------------
   ...
   if (k == 1) then
      ...
      do j=1,ny_block
      do i=1,nx_block
         if (k <= KMT(i,j,bid) .and. k <= KMTE(i,j,bid)) then
            KMASKE(i,j) = c1
         else
            KMASKE(i,j) = c0
         endif
         if (k <= KMT(i,j,bid) .and. k <= KMTN(i,j,bid)) then
            KMASKN(i,j) = c1
         else
            KMASKN(i,j) = c0
         endif

         TEMP(i,j,kn,bid) = max(-c2, TMIX(i,j,k,1))
      end do
      end do

      do j=1,ny_block-1
      do i=1,nx_block-1
         TXP(i,j,kn,bid) = KMASKE(i,j)*(TEMP(i+1,j,kn,bid) - &
                                        TEMP(i  ,j,kn,bid))
         TYP(i,j,kn,bid) = KMASKN(i,j)*(TEMP(i,j+1,kn,bid) - &
                                        TEMP(i,j  ,kn,bid))
      end do
      end do
      ...
      do i=1,nx_block-1
         ...
         TX(i,j,kn,n,bid) = KMASKE(i,j)*(TMIX(i+1,j,k,n) - &
                                         TMIX(i  ,j,k,n))

         TY(i,j,kn,n,bid) = KMASKN(i,j)*(TMIX(i,j+1,k,n) - &
                                         TMIX(i,j  ,k,n))
      ...
      do j=2,ny_block
      do i=2,nx_block
         RXW(i,j,bid) = DRDT(i,j,bid)*TXP(i-1,j,kn,  bid) &
                      + DRDS(i,j,bid)*TX (i-1,j,kn,2,bid)


         RYS(i,j,bid) = DRDT(i,j,bid)*TYP(i,j-1,kn,  bid) &
                      + DRDS(i,j,bid)*TY (i,j-1,kn,2,bid)
      end do
      end do
...
!-----------------------------------------------------------------------
      ...
      do j=1,ny_block-1
      do i=1,nx_block-1
         TXP(i,j,ks,bid) = KMASKE(i,j)*(TEMP(i+1,j,ks,bid) - &
                                        TEMP(i  ,j,ks,bid))
         TYP(i,j,ks,bid) = KMASKN(i,j)*(TEMP(i,j+1,ks,bid) - &
                                        TEMP(i,j  ,ks,bid))
      end do
      end do
      ...
      do j=1,ny_block-1
      do i=1,nx_block-1
         TX(i,j,ks,n,bid) = KMASKE(i,j)*(TMIX(i+1,j,k+1,n) - &
                                         TMIX(i  ,j,k+1,n))
         TY(i,j,ks,n,bid) = KMASKN(i,j)*(TMIX(i,j+1,k+1,n) - &
                                         TMIX(i,j  ,k+1,n))
      ...
      do j=2,ny_block
      do i=2,nx_block
         RXW(i,j,bid) = DRDT(i,j,bid)*TXP(i-1,j,ks,  bid) + &
                        DRDS(i,j,bid)*TX (i-1,j,ks,2,bid)

         RYS(i,j,bid) = DRDT(i,j,bid)*TYP(i,j-1,ks,  bid) + &
                        DRDS(i,j,bid)*TY (i,j-1,ks,2,bid)
      end do
      end do
...
!-----------------------------------------------------------------------
         ...
         case (slope_control_clip)
            ...
            do j=1,ny_block
            do i=1,nx_block
               if (abs(SLX(i,j,n,kc(kk),bid)* &
                       dzw(k)/HUS(i,j,bid)) > slm_r) then
                  SLX(i,j,n,kc(kk),bid) = sign(slm_r*HUS(i,j,bid)* &
                                               dzwr(k), &
                                               SLX(i,j,n,kc(kk),bid))
               endif
            enddo
            ...
            do j=1,ny_block
            do i=1,nx_block
               if (abs(SLY(i,j,n,kc(kk),bid)*dzw(k)/ &
                       HUW(i,j,bid)) > slm_r) then
                  SLY(i,j,n,kc(kk),bid) = sign(slm_r*HUW(i,j,bid)* &
                                               dzwr(k), &
                                               SLY(i,j,n,kc(kk),bid))
               endif
            enddo
...
!-----------------------------------------------------------------------
   ...
   do i=1,nx_block
      if (k == KMT(i,j,bid)) then
         KAPPA  (i,j,kbt ,bid) = c0
         KAPPA  (i,j,kbt2,bid) = c0
         KAPPA_B(i,j,kbt ,bid) = c0
         KAPPA_B(i,j,kbt2,bid) = c0
      endif
   end do
   end do
   ...
   if (partial_bottom_cells) then
      ...
      do j=2,ny_block-1
      do i=2,nx_block-1
         DZTE(i,j) = DZT(i+1,j,k,bid)*dzr(k)
         DZTW(i,j) = DZT(i-1,j,k,bid)*dzr(k)
         DZTN(i,j) = DZT(i,j+1,k,bid)*dzr(k)
         DZTS(i,j) = DZT(i,j-1,k,bid)*dzr(k)
      end do
      end do
      ...
      do i=1,nx_block
         if (k+1 == KMT(i,j,bid)) then
            ...
            KAPPA  (i,j,kbt ,bid) = KAPPA  (i,j,kbt ,bid)* &
                                       (DZT(i,j,kp1 ,bid)*dzr(kp1))
            KAPPA  (i,j,kbt2,bid) = KAPPA  (i,j,kbt2,bid)* &
                                       (DZT(i,j,kp1 ,bid)*dzr(kp1))
            KAPPA_B(i,j,kbt ,bid) = KAPPA_B(i,j,kbt ,bid)* &
                                       (DZT(i,j,kp1 ,bid)*dzr(kp1))
            KAPPA_B(i,j,kbt2,bid) = KAPPA_B(i,j,kbt2,bid)* &
                                       (DZT(i,j,kp1 ,bid)*dzr(kp1))
         endif
      end do
      end do
      ...
      if (k < km) then
         do j=2,ny_block-1
         do i=2,nx_block-1
            DZTEP(i,j) = DZT(i+1,j,kp1,bid)*dzr(kp1)
            DZTWP(i,j) = DZT(i-1,j,kp1,bid)*dzr(kp1)
            DZTNP(i,j) = DZT(i,j+1,kp1,bid)*dzr(kp1)
            DZTSP(i,j) = DZT(i,j-1,kp1,bid)*dzr(kp1)
         end do
         end do
      endif
...
!-----------------------------------------------------------------------
   ...
   if (partial_bottom_cells) then
      do j=1,ny_block-1
      do i=1,nx_block-1
         WORK3(i,j) = (KAPPA(i  ,j,ktp,bid) + KAPPA(i  ,j,kbt,bid))* &
                       DZTE(i  ,j)*DZT(i  ,j,k,bid) +                &
                      (KAPPA(i+1,j,ktp,bid) + KAPPA(i+1,j,kbt,bid))* &
                       DZTW(i+1,j)*DZT(i+1,j,k,bid)
         WORK4(i,j) = (KAPPA(i,j  ,ktp,bid) + KAPPA(i,j  ,kbt,bid))* &
                       DZTN(i,j  )*DZT(i,j  ,k,bid) +                &
                      (KAPPA(i,j+1,ktp,bid) + KAPPA(i,j+1,kbt,bid))* &
   ...
   if (ah_bkg /= c0) then
      ...
      if (partial_bottom_cells) then
         do j=1,ny_block-1
         do i=1,nx_block-1
            WORK6(i,j) = WORK5(i+1,j)*DZTW(i+1,j)*DZT(i+1,j,k,bid)
            WORK7(i,j) = WORK5(i,j+1)*DZTS(i,j+1)*DZT(i,j+1,k,bid)
         end do
         end do
         ...
         do j=1,ny_block-1
         do i=1,nx_block-1
            WORK6(i,j) = WORK5(i+1,j)
            WORK7(i,j) = WORK5(i,j+1)
         end do
         end do
      endif
      ...
      if (ah /= ah_bolus .or. diff_tapering .or. gm_bolus) then
         ...
         if (partial_bottom_cells) then
            ...
            do i=this_block%ib-1,this_block%ie+1
               ...
               FX(i,j) = FX(i,j) - CX(i,j)*               &
                 ( WORK1(i  ,j)*SLX(i  ,j,ieast,ktp,bid)* &
                                 TZ(i  ,j,kn,n,bid)       &
                 + WORK2(i  ,j)*SLX(i  ,j,ieast,kbt,bid)* &
                                 TZ(i  ,j,ks,n,bid)       &
                 + WORK1(i+1,j)*SLX(i+1,j,iwest,ktp,bid)* &
                                 TZ(i+1,j,kn,n,bid)       &
                 + WORK2(i+1,j)*SLX(i+1,j,iwest,kbt,bid)* &
               ...
               FY(i,j) = FY(i,j) - CY(i,j)*                &
                 ( WORK1(i,j  )*SLY(i,j  ,jnorth,ktp,bid)* &
                                 TZ(i,j  ,kn,n,bid)        &
                 + WORK2(i,j  )*SLY(i,j  ,jnorth,kbt,bid)* &
                                 TZ(i,j  ,ks,n,bid)        &
                 + WORK1(i,j+1)*SLY(i,j+1,jsouth,ktp,bid)* &
                                 TZ(i,j+1,kn,n,bid)        &
                 + WORK2(i,j+1)*SLY(i,j+1,jsouth,kbt,bid)* &
...
!-----------------------------------------------------------------------
   ...
   if (gm_bolus) then
      ...
      if (partial_bottom_cells) then
         ...
         do i=1,nx_block-1
            WORK1(i,j) =                                           &
              (KAPPA_B(i  ,j,kbt ,bid)*SLX(i  ,j,ieast,kbt ,bid) + &
               KAPPA_B(i  ,j,kbt2,bid)*SLX(i  ,j,ieast,kbt2,bid) + &
               KAPPA_B(i+1,j,kbt ,bid)*SLX(i+1,j,iwest,kbt ,bid) + &
               KAPPA_B(i+1,j,kbt2,bid)*SLX(i+1,j,iwest,kbt2,bid))* &
               p25*HYX(i,j,bid)*p5*(DZT(i,j,k,bid) + DZT(i,j,kp1,bid))
     
            WORK2(i,j) =                                            &
              (KAPPA_B(i,j  ,kbt ,bid)*SLY(i,j  ,jnorth,kbt ,bid) + &
               KAPPA_B(i,j  ,kbt2,bid)*SLY(i,j  ,jnorth,kbt2,bid) + &
               KAPPA_B(i,j+1,kbt ,bid)*SLY(i,j+1,jsouth,kbt ,bid) + &
               KAPPA_B(i,j+1,kbt2,bid)*SLY(i,j+1,jsouth,kbt2,bid))* &
               p25*HXY(i,j,bid)*p5*(DZT(i,j,k,bid) + DZT(i,j,kp1,bid))
         end do
         end do
      else
         do j=1,ny_block-1
